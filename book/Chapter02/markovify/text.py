...
class Text(object):
    ...
    def __init__(
        ...
        chain: A trained markov.Chain instance for this text, if pre-processed.
    ...
    def make_sentence_with_start(self, beginning, strict=True, **kwargs):
        ...
        If strict == True, then markov will draw its initial inspiration
        only from sentences that start with the specified word/phrase.

        If strict == False, then markov will draw its initial inspiration
...
class NewlineText(Text):
    """
    A (usable) example of subclassing markov.Text. This one lets you markov
